...
*/
        ...
        this.stelle.tableNames.forEach((table) => {
            this.settings.query = this.settings.query.replace(table, table.replace(".", "_"));
        });

        const sql = this.extentSql(this.settings.query, filter, order, limit, offset);
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            Util.executeSQL(kvm.db, sql)
                ...
                .catch((error) => {
                    ...
                    reject({ message: `Fehler beim Löschen der Tabelle: ${tableName} ${error?.message}`, caause: error });
                });
        });
    }
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            Util.executeSQL(kvm.db, sql)
                ...
                .catch((error) => {
                    ...
                    reject({ message: `Fehler beim Löschen der Deltatabelle: ${tableName} ${error?.message}`, caause: error });
                });
        });
    }
    ...
    async sendDeltas(deltas: { rows: { version: string; sql: string }[] }) {
        return new Promise<SendDeltasResponse>(async (resolve, reject) => {
            try {
                ...
                console.info(`Successful written deltas of layer "${this.title}" into File.`);
                ...
                reject({ message: `Fehler beim Erstellen oder Senden der Deltas für den Layer "${this.title}"`, cause: ex });
            }
        });

        // .catch((error) => {
        //     console.log("Failed file write: ", error);
        //     const msg = "Fehler beim Erzeugen der Delta-Datei, die geschickt werden soll.";
    ...
    async _upload(fileEntry: FileEntry): Promise<FileUploadResult> {
        ...
        console.log(`Layer: "${this.title}" going to upload deltas fileURL: "${fileURL}"`);
    ...
    async sendNewImage(img: string) {
        ...
        console.error(`sendNewImage ${img} (Layer: ${this.title})`);
        ...
        console.error("upload to url: " + server);
                    ...
                    .then((json) => {
                        if (json.success) {
                            ...
                            this.clearDeltas("img", img)
                                .then(() => resolve())
                                .catch((err) => {
                                    reject({ message: `clearDeltas(img, ${img}) war nicht erfolgreich.`, cause: err });
                                });
                        } else {
                            reject({ message: `Beim Löschen des Bildes "${img}" meldet der Server nicht erfolreich. Antwort: ${JSON.stringify(json)} ` });
                        }
                    })
                    .catch((reason) => reject({ message: `Die Antwort vom Server auf den Reuest zum LÖschen des Bildes "${img}" konnte nicht geparst werden.`, cause: reason }));
            });
        });
    ...
    async syncData() {
        ...
        ";
                ...
                if (sendDeltasResponse.success) {
                    ...
                    reject({ message: `Negative Antwort auf Upload der Deltas (Layer "${this.title}") ServerErrorMessage:"${sendDeltasResponse.err_msg}"`, response: sendDeltasResponse });
                }
            } catch (ex) {
                reject(ex);
            }
...
*/
                ...
                if ($("#newPosSelect").val() == 1) {
                    navigator.geolocation.getCurrentPosition(
                        function (geoLocation) {
                            ...
                            kvm.activeLayer.startEditing(kvm.activeLayer.getStartGeomAtLatLng(startLatLng), startLatLng);
        ...
        for (let i = 0; i < this.attributes.length; i++) {
            ...
            if (attr.isAutoAttribute(action) && !changesKeys.includes(attr.get("name"))) {
                ...
                try {
                    ...
                    results.push({
                        key: attr.get("name"),
                        oldVal: kvm.activeLayer.activeFeature.getDataValue(attr.get("name")),
                        newVal: autoValue,
                        type: attr.getSqliteType(),
                    });
                } catch ({ name, message }) {
    ...
    readDataset(rs = null) {
        try {
            ...
            const sql = layer.extentSql(layer.settings.query, [`${layer.settings.table_alias}.${id_attribute} = '${featureId}'`, `${layer.settings.table_alias}.endet IS NULL`]);
     ...
     */
    appendToApp() {
        console.error(`xxx appendoApp ${this.getGlobalId()} ${this.title}`);
        ...
        try {
            ...
            if (index == 0) {
                ...
                // document.getElementById("layer_list").insertAdjacentHTML("beforeend", this.getListItem());
                document.getElementById("layer_list").append(this.getLayerListItem());
                console.error("appendToApp append getListItem " + this.getGlobalId());
            }
            console.error("xxx layer_list childCount=" + document.getElementById("layer_list").childNodes.length);
            window.setTimeout(() => {
    ...
    bttnSyncLayerClicked(evt: MouseEvent) {
        console.info(`bttnSyncLayerClicked`);
    ...
    bindLayerEvents(layerGlobalId: any) {
        console.error(`xxx bindLayerEvents ${layerGlobalId}`, this.getGlobalId());
        ...
        // $(".sync-layer-button" + (layerGlobalId > 0 ? "[id='syncLayerButton_" + layerGlobalId + "']" : "")).on("click", (evt) => {
        // rtr
        // Daten Synchronisieren
        document.getElementById("syncLayerButton_" + layerGlobalId).addEventListener("click", (evt) => {
            console.error(`sync-layer-button.clicked  (syncLayerButton_${layerGlobalId}`, evt, this);
            const layer = kvm.activeLayer;
            let target = $(evt.target);
        ...
        document.getElementById("syncImagesButton_" + layerGlobalId).addEventListener("click", (evt) => {
            console.error(`syncImagesButton.clicked ${this.getGlobalId()}`);
            // const layer = kvm.activeLayer;
            // let target = $(evt.target);
        ...
        document.getElementById("clearLayerButton_" + layerGlobalId).addEventListener("click", (evt) => {
        ...
        // $(".reload-layer-button" + (layerGlobalId > 0 ? "[id='reloadLayerButton_" + layerGlobalId + "']" : "")).on("click", (evt) => {
        document.getElementById("reloadLayerButton_" + layerGlobalId).addEventListener("click", (evt) => {
            // var id = (<any>evt.target).value;
            // const layer = kvm.activeLayer;
            // let target = $(evt.target);
    ...
    */
        if (this.hasGeometry) {
            ...
            $("#styleLayerOkButton_" + layerGlobalId).on("click", function (evt) {
                ...
                ["color", "fillcolor", "width", "opacity", "fillOpacity"].forEach(function (formvar) {
                    kvm.activeLayer.settings[formvar] = $("#styleLayerDiv_" + globalLayerId + " input[name='" + formvar + "'], #styleLayerDiv_" + globalLayerId + " select[name='" + formvar + "']").val();
                });
        ...
        document.getElementById("infoLayerButton_" + layerGlobalId).addEventListener("click", (evt) => {
    ...
    createLayerListItemFunction(labelTxt: string, name: string, bttnClass: string, iconClass: string, cb: (ev: MouseEvent) => void) {
        const div = createHtmlElement("div", null, "layer-functions-div");
        const bttn = createHtmlElement("button", div, bttnClass + " settings-button active-button layer-function-button");
        bttn.id = name + "Button_" + this.getGlobalId();
        const menuBttn = createHtmlElement("i", bttn, iconClass);
        menuBttn.ariaHidden = "true";
        menuBttn.addEventListener("click", cb);
        const label = createHtmlElement("label", div);
    ...
    getLayerListItem() {
        ...
        radioInput.addEventListener("change", (evt) => {
            this.layerSelectionClicked(evt);
        });
        (radioInput.name = "activeLayerId"), (radioInput.value = `${this.getGlobalId()}`);
        const label = createHtmlElement("span", div, "layer-list-element");
        label.innerText = this.get("alias") ? this.get("alias") : this.get("title");
        const menuBttn = createHtmlElement("i", div, "layer-functions-button fa-regular fa-ellipsis-vertical");
        ...
        div.append(this.createLayerListItemFunction("Lokale Daten löschen", "clearLayer", "clear-layer-button", "fa fa-ban", (ev) => this.bttnClearLayerClicked(ev)));
        div.append(this.createLayerListItemFunction("Layer neu laden", "reloadLayer", "reload-layer-button", "fa fa-window-restore", (ev) => this.bttnReloadLayerClicked(ev)));

        if (this.hasGeometry) {
            div.append(this.createLayerListItemFunction("Layer Style", "styleLayer", "style-layer-button", "fa fa-paint-brush", (ev) => this.bttnStyleLayerClicked(ev)));
        }
    ...
    getLayerinfoPanel() {
        const div = createHtmlElement("div", null, "layer-functions-div info-layer-div");
        div.style.display = "none";
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Layer-ID: ${this.get("id")}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Geometrietyp: ${this.get("geometry_type")}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Synchronisierbar: ${this.hasEditPrivilege ? "Ja" : "Nein"}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Hinzufügen und Editieren von Datensätzen erlaubt: ${this.hasEditPrivilege ? "Ja" : "Nein"}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Löschen von Datensätzen erlaubt: ${this.hasEditPrivilege ? "Ja" : "Nein"}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Tabellenname: ${this.get("table_name")}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Feld für ID: ${this.get("id_attribute")}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Feld für Geometrie: ${this.get("geometry_attribute")}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Feld für Datensatzbezeichnung: ${this.get("name_attribute")}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Feld für Klassifizierung: ${this.get("classitem")}` });
        const dsCountSpan = createHtmlElement("span", div, "item-info-layer", { innerHTML: `Anzahl Datensätze:` });
        const anzDS = this._features ? this._features.size : 0;
        createHtmlElement("span", dsCountSpan, null, { id: `numDatasetsText_${this.getGlobalId()}`, innerHTML: `${anzDS}` });
        createHtmlElement("span", div, "item-info-layer", { innerHTML: `Layer-Version: ${this.get("version")}` });
        // createHtmlElement("span", div, "item-info-layer", {})
        ...
        if (this.hasEditPrivilege) {
            const syncVersionSpan = createHtmlElement("span", div, "item-info-layer", { innerHTML: `Sync-Version:` });
            createHtmlElement("span", syncVersionSpan, null, { id: `syncVersionSpan_${this.getGlobalId()}`, innerHTML: `${this.get("syncVersion")}` });
    ...
    activate() {
        ...
        try {
            ...
            $("#layer_" + kvm.activeLayer.getGlobalId() + " > .layer-functions-button").removeClass("fa-ellipsis-vertical fa-square-xmark");
            $("#layer_" + kvm.activeLayer.getGlobalId() + " > .layer-functions-button").addClass("fa-ellipsis-vertical");
            if (this.hasEditPrivilege) {
