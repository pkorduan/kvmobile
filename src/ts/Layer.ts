...
*/
     ...
     */
    // TODO Bug?
    this.readData = function (limit = 50000, offset = 0, order: any = "") {
        ...
        filter = $(".filter-view-field")
            ...
            .map(function (i, field) {
                ...
                return $(field).attr("name") + " " + $(field).find(".filter-view-operator select").val() + " " + bracket + $(field).find(".filter-view-value-field").val() + bracket;
            })
            .get();
    ...
    this.createFeatureList = function () {
        ...
        if (Object.keys(this.features).length > 0) {
            ...
            $("#numDatasetsText").html(Object.keys(this.features).length.toString()).show();
        }
    };
...
*/
    ...
    this.collectChanges = function (action) {
        ...
        changes = $.map(
            ...
            function (attr) {
                ...
                if (attr.get("name") != this.id_attribute && !attr.isAutoAttribute(action) && attr.get("privilege") != "0") {
                    var key = attr.get("name"),
    ...
    this.bindLayerEvents = function (layerGlobalId = 0) {
        ...
        $(".reload-layer-button" + (layerGlobalId > 0 ? "[id='reloadLayerButton_" + layerGlobalId + "']" : "")).on("click", function (evt) {
            ...
            if (!layer.isEmpty()) {
                navigator.notification.confirm("Layer ist noch nicht geleert. Die Daten des Layers auf dem Endgerät müssen erst gelöscht werden.", function (buttonIndex) {}, "Datenbank", ["OK"]);
            } else {
                navigator.notification.confirm(
...
*/
    ...
    this.getListItem = function () {
      ...
      <div id="layer_' +
            this.getGlobalId() +
            '">\
        <input type="radio" name="activeLayerId" value="' +
            this.getGlobalId() +
            '"/> ' +
            (this.get("alias") ? this.get("alias") : this.get("title")) +
            '\
        <i id="layer-functions-button_' +
            this.getGlobalId() +
            '" class="layer-functions-button fa fa-ellipsis-v" aria-hidden="true"></i>\
        <div class="layer-functions-div">\
          <button id="syncLayerButton_' +
            this.getGlobalId() +
            '" value="' +
            this.getGlobalId() +
    ...
    this.getImgDownloadUrl = function (image) {
        ...
        url += file + "Stelle_ID=" + stelle.get("Stelle_ID") + "&" + "login_name=" + stelle.get("login_name") + "&" + "passwort=" + encodeURIComponent(stelle.get("passwort")) + "&" + "go=mobile_download_image" + "&" + "image=" + image;
        return url;
    };
  ...
  */
    ...
    this.notNullValid = function () {
        ...
        $.each(kvm.activeLayer.attributes, function (i, v) {
            if (!v.isAutoAttribute("") && v.get("nullable") == 0 && v.formField.getValue() == null) {
