...
export class Stelle {
    ...
    fileTransfer.download(
      ...
      function (fileEntry) {
        fileEntry.file(
          function (file) {
            ...
            reader.onloadend = function () {
              ...
              if (
                ...
                this.result.indexOf('form name="login"') == -1
              ) {
                if (kvm.isValidJsonString(this.result)) {
                  ...
                  if (resultObj.success) {
                    ...
                    console.log(
                      "Download erfolgreich. Antwortobjekt: %o",
                      resultObj
                    );
                    ...
                    $.each(resultObj.stellen, function (index, stelle) {
                      $("#kvwmapServerStelleSelectField").append(
                        '<option value="' +
                          stelle.ID +
                          '">' +
                          stelle.Bezeichnung +
                    ...
                    $("#kvwmapServerStellenField").val(
                      JSON.stringify(resultObj.stellen)
                    );
                    $("#requestStellenButton").hide();
                    if (resultObj.stellen.length == 1) {
                      $("#kvwmapServerStelleSelectField").val(
                        resultObj.stellen[0].ID
                      );
                    ...
                    errMsg =
                      "Fehler beim Abfragen der Stellendaten. " +
                      (resultObj.err_msg ? resultObj.err_msg : "");
                  }
                } else {
                  errMsg =
          ...
          function (error) {
            kvm.msg(
              "Fehler beim Einlesen der heruntergeladenen Datei. Prüfen Sie die URL und Parameter, die für den Download verwendet werden."
            );
  ...
  getStellenUrl() {
    ...
    url +=
      ...
      this.get("login_name") +
      "&passwort=" +
      encodeURIComponent(this.get("passwort")) +
      "&format=json";
    return url;
  }
  ...
  finishLayerLoading(layer) {
    console.log(
      "finishLayerReading: readAllLayers= %s, numLayersLoaded=%s, numLayers=%s",
      this.readAllLayers,
      ...
      if (this.numLayersLoaded < this.numLayers - 1) {
        ...
        kvm.tick(
          `${layer.title}:<br>&nbsp;&nbsp;${
            ...
            this.numLayers - this.numLayersLoaded
          } Layer zu laden.`
        );
      } else {
  ...
  finishLayerReading(layer) {
    console.log(
      "finishLayerReading: readAllLayers= %s, numLayersRead=%s, numLayers=%s",
      this.readAllLayers,
      this.numLayersRead,
      this.numLayers
    );
      ...
      if (this.numLayersRead < this.numLayers - 1) {
        ...
        kvm.tick(
          `${layer.title}:<br>&nbsp;&nbsp;${
            ...
            this.numLayers - this.numLayersRead
          } Layer zu laden.`
        );
      } else {
        kvm.tick(`${layer.title}:<br>&nbsp;&nbsp;Laden beeendet.`);
        ...
        this.readAllLayers = false;
        const globalLayerId = `${kvm.store.getItem(
          "activeStelleId"
        )}_${kvm.store.getItem("activeLayerId")}`;
    ...
    console.log(
      ...
      kvm.activeLayer ? kvm.activeLayer.get("id") : "keiner aktiv"
    );
  }
  ...
  getTableNames() {
    ...
    return layerIds
      .map((layerId) => {
        try {
          let layerSettings = JSON.parse(
            kvm.store.getItem(`layerSettings_${stelleId}_${layerId}`)
          );
  ...
  reloadLayer(layerId) {
    ...
    console.log(
      "Download Layerdaten von Stelle mit url: %s",
      kvm.replacePassword(url)
    );
    //kvm.log("Speicher die Datei auf dem Gerät in Datei: " + filename);
        ...
        fileEntry.file(
          function (file) {
            ...
            reader.onloadend = function () {
              ...
              if (resultObj.success) {
                kvm.tick(
                  `${kvm.activeLayer.title}:<br>&nbsp;&nbsp;Download erfolgreich.`
                );
                ...
                let layer;
                kvm.tick(
                  `${kvm.activeLayer.title}:<br>&nbsp;&nbsp;Entferne Layer von App.`
                );
  ...
  getLayerDrawingIndex(layer: Layer | MapLibreLayer) {
    ...
    // TODO CHECK !!
    let index = sortedLayers.findIndex(
      (layer, index, layers) =>
        parseInt(layer.get("drawingorder")) >
        parseInt(layer.get("drawingorder"))
    );
    ...
    console.log(
      "%s: Stelle.getLayerDrawingIndex return index: %s",
      layer.get("title"),
      index
    );
    return index;
  ...
  requestLayers() {
    ...
    console.log(
      "Download Layerdaten der Stelle mit url: ",
      kvm.replacePassword(url)
    );
        ...
        fileEntry.file(
          (file) => {
            ...
            reader.onloadend = (evt) => {
              ...
              const resultObj = <any>(
                kvm.parseLayerResult(<string>evt.target.result)
              );
              ...
              if (resultObj.success) {
                ...
                if ("layerIds_" + kvm.activeStelle.get("id") in kvm.store) {
                  const layerIds = JSON.parse(
                    kvm.store["layerIds_" + kvm.activeStelle.get("id")]
                  );
                ...
                if (this.numLayers > 0) {
                  ...
                  resultObj.layers.sort((a, b) =>
                    parseInt(a.drawingorder) > parseInt(b.drawingorder) ? 1 : -1
                  );

                  // add requested layers
                  console.log(
                    "  requestLayers) Füge neu runtergeladene Layer zur Anwendung hinzu."
                  );
                  resultObj.layers.forEach((layerSetting) => {
                    console.log(
                      "  requestLayers) Layer.requestLayers create layer with settings: %o",
                      layerSetting
                    );
  ...
  sortOverlays() {
    kvm.controls.layers._layers
      .filter((l) => l.overlay)
      .sort((a, b) =>
        parseInt(a.layer.getAttribution()) > parseInt(b.layer.getAttribution())
          ? 1
          : -1
      )
      .forEach((overlay) => {
  ...
  getLayerUrl(options = { hidePassword: false }) {
    ...
    url += `${file}go=mobile_get_layers&login_name=${this.get(
      "login_name"
    )}&passwort=${
      options.hidePassword ? "*****" : encodeURIComponent(this.get("passwort"))
    }&Stelle_ID=${this.get("Stelle_ID")}&kvmobile_version=${
      kvm.versionNumber
    }&format=json`;
    return url;
  }
