...
export function Stelle(settings = {}) {
    ...
    this.getLayerUrl = function (options = { hidePassword: false }) {
        ...
        url += file + "go=mobile_get_layers" + "&" + "login_name=" + this.get("login_name") + "&" + "passwort=" + (options.hidePassword ? "*****" : encodeURIComponent(this.get("passwort"))) + "&" + "Stelle_ID=" + this.get("Stelle_ID");
        return url;
    };
    ...
    this.allLayerLoaded = function () {
        ...
        $.each(this.layerIds, function (key, layerId) {
            // TODO
            var layerSettings = <any>kvm.store.getItem("layerSettings_" + layerId);
