...
*/
        ...
        if (this.store.getItem("activeStelleId") && this.store.getItem(`stelleSettings_${this.store.getItem("activeStelleId")}`) != null) {
            console.log(this.config.name, this.store.getItem("activeStelleId"), JSON.parse(this.store.getItem(`stelleSettings_${this.store.getItem("activeStelleId")}`)).Stelle_ID);
        }
    ...
    backupDatabase(filename = "") {
        ...
        kvm.controller.files.copyFile(srcDir, srcFile, dstDir, dstFile, `Datenbank erfolgreich gesichert in: ${dstDir} ${dstFile}`);
    }
