  ...
  this.setValue = function (val) {
    ...
    if (val && this.get("type") == "timestamp") {
        ...
        for (i = 0; i < images.length; i++) {
          ...
          window.resolveLocalFileSystemURL(
            ...
            function () {
              ...
              var img_div = $(
                '<div class="img" src="img/no_image.png" style="background-image: url(img/no_image.png);" field_id="' +
                  this.context.get("index") +
                  '"name="img/no_image.png"></div>'
              );
      ...
      var options = this.get("options").split(";")[0].split(","),
        ...
        subFormTable = JSON.parse(kvm.store.getItem("layerSettings_" + stelleId + "_" + subFormLayerId)).table_name;
      let sql = "\
        SELECT\
        FROM\
          " + subFormTable + "\
