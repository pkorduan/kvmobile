...
export class ObservableSupport<T extends ObservableEvent> {
    ...
    removeEventListener(a: string | Listener<T>, listener?: Listener<T>) {
        let prop: string;
        let lst: Listener<T> | undefined = undefined;
        let lstn: Listener<T>[] | undefined = undefined;
    ...
    fire(event: ObservableEvent) {
        if (this.mapPropToLstn) {
            const listeners = this.mapPropToLstn.get(event.type);
            if (listeners) {
                this._fire(event, listeners);
            }
        }
