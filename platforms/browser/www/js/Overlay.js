function Overlay(stelle, settings = {}) {
    ...
    */
    ...
    this.getOverlayCircleMarkerStyle = function (geoJsonFeature) {
        ...
        var s = this.settings.classes[0].style, d = {
            color: '#284084',
            fill: true,
            fillOpacity: 0.5,
            fillColor: '#3a5dbf',
    ...
    */
    ...
    this.reloadData = function () {
        ...
        var fileTransfer = new FileTransfer(), globalId = this.globalId, filename = 'data_overlay_' + globalId + '.json', url = this.getUrl(), title = this.getTitle(), this_ = this;
    ...
    this.bindOverlayEvents = function () {
        ...
        $('#syncOverlayButton_' + this.globalId).on('click', function (evt) {
            ...
            var target = $(evt.currentTarget), globalId = target.val(), overlay = kvm.overlays[globalId];
            ...
            if (target.hasClass('inactive-button')) {
                ...
                $('#sperr_div').show();
                navigator.notification.confirm('Overlay vom Server holen und Daten in der App aktualisieren?', function (buttonIndex) {
                    if (buttonIndex == 1) { // ja
                ...
                }, 'Aktualisierung Overlay', ['ja', 'nein']);
            }
        });
    };
