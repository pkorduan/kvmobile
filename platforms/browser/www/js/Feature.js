...
function Feature(data = {}, options = {
    ...
    */
    this.setLatLngs = function (geom) {
        ...
        if (this.editableLayer) {
            var newLatLngs = this.wkxToLatLngs(geom), oldLatLngs = this.getLatLng();
    ...
    */
    this.aLatLngsToWkx = function (alatlngs) {
        ...
        switch (this.options.geometry_type) {
            case 'Point':
                ...
                break;
            case 'MultiPoint':
                result = kvm.wkx.Geometry.parse('SRID=4326;MULTIPOINT(' + alatlngs.map(function (point) { return point.lng + ' ' + point.lat; }).join(', ') + ')');
                break;
            case 'Line':
                result = kvm.wkx.Geometry.parse('SRID=4326;LINESTRING(' + alatlngs.map(function (point) { return point.lng + ' ' + point.lat; }).join(', ') + ')');
                break;
            case 'MultiLinestring':
                result = kvm.wkx.Geometry.parse('SRID=4326;MULTILINESTRING(' + alatlngs.map(function (linestring) { return '(' + linestring.map(function (point) { return point[1] + ' ' + point[0]; }).join(', ') + ')'; }).join(', ') + ')');
                break;
            case 'Polygon':
                {
                ...
                break;
            case 'MultiPolygon':
                result = kvm.wkx.Geometry.parse('SRID=4326;MULTIPOLYGON(' + alatlngs.map(function (polygon) { return '(' + polygon.map(function (polyline) { return '(' + polyline.map(function (point) { return point[1] + ' ' + point[0]; }).join(', ') + ')'; }).join(', ') + ')'; }).join(',') + ')');
                break;
    ...
    this.listElement = function () {
        ...
        var markerStyles = JSON.parse(kvm.store.getItem('markerStyles')), numStyles = Object.keys(markerStyles).length, markerStyleIndex = ((this.get('status') >= 0 && this.get('status') < numStyles) ? this.get('status') : 0);
        return '\
    ...
    this.getLabelValue = function () {
        var layer = kvm.activeLayer, label_attribute = layer.get('name_attribute'), formField = layer.attributes[layer.attribute_index[label_attribute]].formField, label_value = '';
    ...
    */
    ...
    this.getNormalPolylineStyle = function () {
        ...
        var markerStyles = JSON.parse(kvm.store.getItem('markerStyles')), numStyles = Object.keys(markerStyles).length, markerStyleIndex = ((this.get('status') >= 0 && this.get('status') < numStyles) ? this.get('status') : 0), style = markerStyles[markerStyleIndex];
