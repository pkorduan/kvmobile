function Attribute(layer, settings = {}) {
    ...
    this.getSqliteType = function () {
        var pgType = this.get('type'), slType = '';
        switch (true) {
            case ($.inArray(pgType, [
                'character varying',
                'text',
    ...
    */
    this.isAutoAttribute = function (action) {
        ...
        /*
            ...
            */
        var answer = (['user_name', 'updated_at_client', 'created_at'].includes(this.get('name')) ||
    ...
    this.toSqliteValue = function (pgType, pgValue) {
        ...
        var slType = this.getSqliteType(), maxByte = 10485760; // 10 MB
        switch (true) {
            case (pgValue == null):
                slValue = 'null';
                break;
    ...
    this.withLabel = function () {
        var label = $('<div class="form-label">').append('<label for="' + this.formField.get('name') + '"/>')
            .append((this.formField.get('alias') ? this.formField.get('alias') : this.formField.get('name'))), value = $('<div class="form-value">');
