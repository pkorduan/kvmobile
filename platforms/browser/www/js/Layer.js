function Layer(stelle, settings = {}) {
    ...
    */
    ...
    this.isEmpty = function () {
        ...
        return (typeof this.get('syncVersion') == 'undefined' ||
    ...
    */
    this.readData = function (limit = 50000, offset = 0, order = '') {
        ...
        $('#sperr_div').show();
        var filter = [], order = $('#anzeigeSortSelect').val(), sql = '';
    ...
    ";
        ...
        kvm.db.executeSql(sql, [], (function (rs) {
    ...
    */
    this.createFeatureList = function () {
        ...
        $.each(this.features, function (key, feature) {
            ...
            var needle = $('#searchHaltestelle').val().toLowerCase(), element = $(feature.listElement()), haystack = element.html().toLowerCase();
    ...
    this.writeData = function (items) {
        ...
        debug_is = items;
        var tableName = this.get('schema_name') + '_' + this.get('table_name'), keys = this.getTableColumns().join(', '), values = '(' +
            $.map(items, function (item) {
                //console.log('item.geometry %', item.geometry);
                if (item.geometry) {
                    return $.map(kvm.activeLayer.attributes.filter(function (attr) {
                        return attr.get('saveable') == '1';
                    }), (function (attr) {
                        var type = attr.get('type'), value = (type == 'geometry' ? this.item.geometry : this.item.properties[attr.get('name')]);
    ...
    this.createTables = function () {
        ...
        var layerIds = $.parseJSON(kvm.store.getItem('layerIds_' + this.stelle.get('id'))), layer_ = this, i;
    ...
    this.createTable = function () {
        kvm.db.transaction((function (tx) {
            ...
            tx.executeSql(sql, [], (function (tx, res) {
                ...
                var tableName = this.get('schema_name') + '_' + this.get('table_name') + '_deltas', tableColumnDefinitions = [
    ...
    this.updateTable = function () {
        ...
        kvm.db.transaction((function (tx) {
            var tableName = this.get('schema_name') + '_' + this.get('table_name'), sql = 'DROP TABLE IF EXISTS ' + tableName;
    ...
    */
    this.getTableColumnDefinitions = function () {
        var tableColumnDefinitions = $.map(this.attributes.filter(function (attr) {
            return attr.get('saveable') == '1';
        }), function (attr) {
    ...
    this.getTableColumns = function () {
        ...
        var tableColumns = $.map(this.attributes.filter(function (attr) {
            return attr.get('saveable') == '1';
        }), function (attr) {
            return attr.get('name');
        });
    ...
    this.getColumnValues = function () {
        ...
        var values = $.map(this.attributes.filter(function (attr) {
            return attr.get('saveable') == '1';
        }), function (attr) {
            var type = attr.get('type'), value = this.activeFeature.get(attr.get('name'));
            v = attr.toSqliteValue(type, value);
            return v;
        });
    ...
    */
    this.getCreateTableSql = function () {
        ...
        var tableName = this.get('schema_name') + '_' + this.get('table_name'), tableColumnDefinitions = this.getTableColumnDefinitions(), sql = 'CREATE TABLE IF NOT EXISTS ' + tableName + ' (' + tableColumnDefinitions.join(', ') + ', endet TEXT)';
        return sql;
    };
    /**
    ...
    */
    ...
    this.requestData = function () {
        ...
        var fileTransfer = new FileTransfer(), filename = 'data_layer_' + this.getGlobalId() + '.json', url = this.getSyncUrl();
        kvm.log('Speicher die Daten in Datei: ' + cordova.file.dataDirectory + filename, 3);
        fileTransfer.download(url, cordova.file.dataDirectory + filename, function (fileEntry) {
            fileEntry.file(function (file) {
                var reader = new FileReader();
                reader.onloadend = function () {
                    kvm.log('Download der Daten ist abgeschlossen.', 3, true);
                    var items = [], collection = {}, errMsg = '';
    ...
    this.upload = function (fileEntry) {
        ...
        var success = (function (r) {
            ...
            if (response.success) {
                ...
                if (this.numReturnedDeltas > 0) {
                    $.each(response.deltas, (function (index, value) {
                        if (kvm.coalesce(value.sql, '') != '') {
                            ...
                            this.execSql(this.pointToUnderlineName(value.sql, this.get('schema_name'), this.get('table_name')), (this.execServerDeltaSuccessFunc).bind({ context: this, response: response, numReturnedDeltas: this.numReturnedDeltas }));
                        }
                    }).bind(this));
                }
                else {
        ...
        var layer = kvm.activeLayer, stelle = layer.stelle, url = stelle.get('url'), file = stelle.getUrlFile(url), server = url + file, params = {}, options = new FileUploadOptions();
    ...
    */
    ...
    ";
        ...
        kvm.db.executeSql(sql, [], (function (rs) {
            ...
            var numRows = rs.rows.length, icon, i;
            if (numRows > 0) {
    ...
    this.sendNewImage = function (img) {
        ...
        var icon = $('#syncImagesIcon_' + this.getGlobalId()), ft = new FileTransfer(), fileURL = 'file://' + config.localImgPath + img.substring(img.lastIndexOf('/') + 1), url = this.stelle.get('url'), file = this.stelle.getUrlFile(url), server = url + file, win = (function (r) {
            ...
            kvm.msg(msg);
            kvm.log(msg);
        }).bind(icon), options = new FileUploadOptions();
    ...
    */
    ...
    ";
        ...
        kvm.db.executeSql(sql, [], (function (rs) {
            ...
            var numRows = rs.rows.length, deltas = { 'rows': [] }, i;
            for (i = 0; i < numRows; i++) {
    ...
    */
    ...
    ';
        kvm.db.executeSql(sql, [], (function (rs) {
            ...
            */
            ...
            navigator.notification.confirm('Fehler bei Löschen der Layerdaten!\nFehlercode: ' + error.code + '\nMeldung: ' + error.message, function (buttonIndex) {
    ...
    ';
        ...
        kvm.db.executeSql(sql, [], (function (rs) {
            ...
            */
            ...
            if (icon.hasClass('fa-spinner'))
                icon.toggleClass('fa-ban fa-spinner fa-spin');
            navigator.notification.confirm('Fehler bei Löschen der Änderungsdaten des Layers!\nFehlercode: ' + error.code + '\nMeldung: ' + error.message, function (buttonIndex) {
                // ToDo handling choices after error
            }, 'Datenbank', ['OK']);
        });
    };
    this.downloadImage = function (localFile, remoteFile) {
        var fileTransfer = new FileTransfer(), downloadURL = this.getImgDownloadUrl(remoteFile);
        kvm.log('Download Datei von URL: ' + downloadURL, 3);
        kvm.log('Speicher die Datei auf dem Gerät in Datei: ' + localFile, 3);
        fileTransfer.download(downloadURL, localFile, (function (fileEntry) {
    ...
    this.createDataView = function () {
      ...
      <div id="dataViewDiv"></div>');
        $.map(this.attributes.filter(function (attribute) { return attribute.get('type') != 'geometry'; }), function (attr) {
            $('#dataViewDiv').append(attr.viewField.withLabel()).append('<div style="clear: both">');
    ...
    */
    this.loadFeatureToView = function (feature, options = {}) {
        ...
        $.map(this.attributes.filter(function (attribute) { return attribute.get('type') != 'geometry'; }), (function (attr) {
            var key = attr.get('name'), val = this.get(key);
    ...
    */
    this.loadFeatureToForm = function (feature, options = { editable: false }) {
        ...
        $.map(this.attributes, (function (attr) {
            var key = attr.get('name'), val = this.get(key), req_by_idx;
            ...
            if (kvm.coalesce(attr.get('req_by'), '') != '') {
                ...
                kvm.activeLayer.attributes[req_by_idx].formField.filter_by_required(attr.get('name'), val);
            }
        }).bind(feature));
    ...
    this.loadTplFeatureToForm = function (tplId) {
        ...
        $.map(this.attributes, (function (attr) {
            var key = attr.get('name'), val = this.get(key); // this is here the tplFeature
        ...
        $.each(this.features, (function (key, feature) {
            ...
            if (feature.newGeom) {
                ...
                vectorLayer.on('click', function (evt) {
                    if (evt.target.options.featureId in kvm.activeLayer.features &&
    ...
    */
    this.newFeature = function () {
        ...
        this.activeFeature = new Feature('{ "' + this.get('id_attribute') + '": "' + kvm.uuidv4() + '", "version": "' + ((this.get('syncVersion') == 'null' ? 0 : this.get('syncVersion')) + 1) + '"}', {
            id_attribute: 'uuid',
            geometry_type: this.get('geometry_type'),
            geometry_attribute: this.get('geometry_attribute'),
            globalLayerId: this.getGlobalId(),
        ...
        if (this.activeFeature.geom) {
            ...
            if ($('#newPosSelect').val() == 1) {
                navigator.geolocation.getCurrentPosition(function (geoLocation) {
                    ...
                    navigator.notification.confirm('Da keine GPS-Position ermittelt werden kann, wird die neue Geometrie in der Mitte der Karte gezeichnet. Schalten Sie die GPS Funktion auf Ihrem Gerät ein und suchen Sie einen Ort unter freiem Himmel auf um GPS benutzen zu können.', function (buttonIndex) {
                        if (buttonIndex == 1) {
                            kvm.log('Einschalten der GPS-Funktion.', 3);
                        }
                    }, 'GPS-Position', ['ok', 'ohne GPS weitermachen']);
                }, {
                    maximumAge: 2000,
                    timeout: 5000,
    ...
    this.getStartGeomAtLatLng = function (latlng) {
        var startGeomSize = 0.0002, startGeom;
    ...
    */
    this.startEditing = function (alatlng = []) {
        ...
        var feature = this.activeFeature, vectorLayer;
        if (alatlng.length > 0) {
    ...
    */
    ...
    this.collectChanges = function (action) {
        ...
        var activeFeature = this.activeFeature, changes = [];
        ...
        changes = $.map(this.attributes, (function (attr) {
            //console.log('attr name: %s', attr.get('name'));
            //console.log('attr.privilege: %s', attr.get('privilege'));
            if (attr.get('name') != this.id_attribute &&
                !attr.isAutoAttribute(action) &&
                attr.get('privilege') != '0') {
                var key = attr.get('name'), oldVal = activeFeature.get(key), newVal = attr.formField.getValue(action);
    ...
    */
    this.backupDataset = function () {
        ...
        var layer = this.context, table = layer.get('schema_name') + "_" + layer.get('table_name'), tableColumns = layer.getTableColumns(), id_attribute = layer.get('id_attribute'), id = layer.activeFeature.get(id_attribute), sql = "\
          INSERT INTO " + table + "(" +
            tableColumns.join(', ') + ",\
            endet\
          )\
    ...
    this.createDataset = function (rs) {
        ...
        var layer = this.context, changes = layer.collectChanges('insert'), imgChanges = changes.filter(function (change) {
    ...
    */
    ...
    this.updateDataset = function (rs) {
        ...
        var layer = this.context, changes = layer.collectChanges('update'), delta = {}, sql = '';
        if (changes.length == 0) {
            $('#sperr_div').hide();
    ...
    */
    this.addAutoChanges = function (changes, action) {
        ...
        var changesKeys = $.map(changes, function (change) {
            ...
            if (attr.isAutoAttribute(action) &&
    ...
    */
    ...
    this.deleteDataset = function (rs) {
        ...
        var layer = this.context, delta = layer.getDeleteDelta(), sql = delta.delta + " AND endet IS NULL";
        this.next.context = layer;
        this.next.delta = delta;
    ...
    */
    this.afterDeleteDataset = function (rs) {
        ...
        var layer = this.context, featureId = layer.activeFeature.id, layerId = layer.activeFeature.layerId;
    ...
    */
    this.writeDelta = function (rs) {
        ...
        var layer = this.context, delta = this.delta, changes = this.changes, sql = "\
    ...
    this.restoreDataset = function (rs) {
        ...
        var layer = this.context, table = layer.get('schema_name') + '_' + layer.get('table_name'), id_attribute = layer.get('id_attribute'), id = layer.activeFeature.get(id_attribute), sql = "\
          UPDATE " + table + "\
          SET endet = NULL\
          WHERE\
            " + id_attribute + " = '" + id + "' AND\
            endet IS NOT NULL\
        ", changes = $.map(layer.attributes.filter(function (attr) {
            return attr.get('saveable') == '1';
        }), function (attr) {
    ...
    */
    this.readDataset = function (rs = null) {
        ...
        //console.log('layer: %o', this.context);
        var layer = this.context, id_attribute = layer.get('id_attribute'), id = layer.activeFeature.get(id_attribute), sql = "\
          SELECT\
    ...
    */
    this.execSql = function (sql, successFunc) {
        ...
        kvm.db.executeSql(sql, [], (successFunc).bind(this), function (error) {
            navigator.notification.confirm('Fehler bei der Speicherung der Änderungen aus dem Formular!\nFehlercode: ' + error.code + '\nMeldung: ' + error.message, function (buttonIndex) {
                // ToDo handling choices after error
            }, 'Datenbank', ['Abbruch']);
        });
    };
    ...
    */
    this.bindLayerEvents = function (layerGlobalId = 0) {
        ...
        //
        $('input[name=activeLayerId]' + (layerGlobalId > 0 ? "[value='" + layerGlobalId + "']" : '')).on('change', function (evt) {
            var globalId = evt.target.value, layer = kvm.layers[globalId];
            //        layer.readData();
        ...
        $('.sync-layer-button' + (layerGlobalId > 0 ? "[id='syncLayerButton_" + layerGlobalId + "']" : '')).on('click', function (evt) {
            var layer = kvm.activeLayer, target = $(evt.target);
            ...
            if (target.hasClass('inactive-button')) {
                ...
                if (layer.runningSyncVersion == 0) {
                    navigator.notification.confirm('Daten vom Server holen. Danach können eigene Änderungen synchronisiert werden.', function (buttonIndex) {
                        if (buttonIndex == 1) { // ja
                            layer.requestData();
                        }
                        else {
                    ...
                    }, 'Layer mit Server synchronisieren', ['ja', 'nein']);
                }
                else {
                    navigator.notification.confirm('Jetzt lokale Änderungen, fallse vorhanden, zum Server schicken und Änderungen vom Server holen und lokal einspielen?', function (buttonIndex) {
                        if (buttonIndex == 1) { // ja
                            layer.syncData();
                        }
                        else {
        ...
        $('.sync-images-button' + (layerGlobalId > 0 ? "[id='syncImagesButton_" + layerGlobalId + "']" : '')).on('click', function (evt) {
            ...
            if (target.hasClass('inactive-button')) {
                ...
                navigator.notification.confirm('Bilder mit Server Synchronisieren?', function (buttonIndex) {
                    if (buttonIndex == 1) { // ja
        ...
        $('.clear-layer-button' + (layerGlobalId > 0 ? "[id='clearLayerButton_" + layerGlobalId + "']" : '')).on('click', function (evt) {
            var id = evt.target.value, layer = kvm.activeLayer;
            if (layer.isEmpty()) {
                navigator.notification.confirm('Layer ist schon geleert!', function (buttonIndex) {
                }, 'Datenbank', ['OK']);
            }
            else {
                navigator.notification.confirm('Alle lokale Daten und nicht hochgeladene Änderungen wirklich Löschen?', function (buttonIndex) {
                    if (buttonIndex == 1) { // ja
        ...
        $('.reload-layer-button' + (layerGlobalId > 0 ? "[id='reloadLayerButton_" + layerGlobalId + "']" : '')).on('click', function (evt) {
            var id = evt.target.value, layer = kvm.activeLayer;
            if (!layer.isEmpty()) {
                navigator.notification.confirm('Layer ist noch nicht geleert. Die Daten des Layers auf dem Endgerät müssen erst gelöscht werden.', function (buttonIndex) {
                }, 'Datenbank', ['OK']);
            }
            else {
                navigator.notification.confirm('Die Einstellungen des Layers neu laden und die Tabelle neu anlegen.', function (buttonIndex) {
                    if (buttonIndex == 1) { // ja
                        var layer = kvm.activeLayer;
    ...
    */
    ...
    this.getSyncUrl = function () {
        ...
        var url = this.stelle.get('url'), file = this.stelle.getUrlFile(url);
        url += file +
            'Stelle_ID=' + this.stelle.get('Stelle_ID') + '&' +
            'login_name=' + this.stelle.get('login_name') + '&' +
            'selected_layer_id=' + this.get('id') + '&' +
    ...
    this.saveToStore = function () {
        ...
        var layerIds = $.parseJSON(kvm.store.getItem('layerIds_' + this.stelle.get('id'))), settings = JSON.stringify(this.settings);
        if (layerIds == null) {
            layerIds = [];
        }
    ...
    this.createLayerFilterForm = function () {
        ...
        $.each(this.attributes, function (key, value) {
            if (value.settings.type != 'geometry') {
                ...
                $('#attributeFilterFieldDiv').append($('<div class="filter-view-field" database_type="' + value.settings.type + '" name="' + value.settings.name + '">')
