...
function DataViewField(divId, settings) {
    ...
    this.setValue = function (val) {
        ...
        if (val && this.get('type') == 'timestamp') {
            ...
            var options = this.get('options').split(';')[0].split(','), stelleId = kvm.activeStelle.get('id'), subFormLayerId = options[0], subFormFK = options[1], subFormPreviewAttribute = options[2], subFormTable = $.parseJSON(kvm.store.getItem('layerSettings_' + stelleId + '_' + subFormLayerId)).table_name;
            sql = "\
        SELECT\
        FROM\
          " + subFormTable + "\
...
//# sourceMappingURL=DataViewField.js.map