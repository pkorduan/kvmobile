function BilderFormField(formId, settings) {
    ...
    */
    this.setValue = function (val) {
        ...
        var val = kvm.coalesce(val, ''), images, localFile, remoteFile, i;
    ...
    */
    this.addImage = function (src, name = '') {
        ...
        var name = (name == '' ? src : name), img_div = $('<div class="img" src="' + src + '" style="background-image: url(' + src + ');" field_id="' + this.get('index') + '"name="' + name + '"></div>');
        ...
        */
        ...
        $('div[name$="' + name + '"]').on('click', function (evt) {
            var target = $(evt.target), src = target.attr('src'), fieldId = target.attr('field_id');
            if (src == 'img/no_image.png') {
                navigator.notification.confirm('Bild herunterladen?', function (buttonIndex) {
                    if (buttonIndex == 1) { // ja
                        var remoteFile = target.attr('name'), localFile = kvm.activeLayer.attributes[fieldId].formField.serverToLocalPath(remoteFile);
                ...
                cordova.plugins.fileOpener2.open(src, 'image/jpeg', {
                    ...
                    success: function () {
                        ...
                        navigator.notification.confirm('Bild Löschen?', function (buttonIndex) {
                            if (buttonIndex == 1) { // ja
    ...
    */
    this.dropImage = function (imgDiv) {
        var imageField = this.element, src = imgDiv.attr('src'), activeLayer = kvm.activeLayer, sql = '';
        ...
        imageField.val(kvm.addBraces($.map(kvm.removeBraces(imageField.val()).split(','), function (path) {
    ...
    this.dropAllPictures = function (evt) {
        ...
        navigator.notification.confirm('Wirklich alle Bilder in diesem Datensatz Löschen?', function (buttonIndex) {
            if (buttonIndex == 1) { // ja
                context.setValue('');
    ...
    */
    this.takePicture = function (evt) {
        ...
        navigator.camera.getPicture((function (cameraPicture) {
        ...
        }).bind(evt.data.context), function (message) {
    ...
    */
    this.moveFile = (function (srcFile, dstDir) {
        ...
        window.resolveLocalFileSystemURL(dstDir, (function success(dirEntry) {
            ...
            window.resolveLocalFileSystemURL(srcFile, (function success(fileEntry) {
...
//# sourceMappingURL=BilderFormField.js.map