kvm.controller.files = {
    ...
    writeTextToFile: function (fileName, content) {
        window.requestFileSystem(LocalFileSystem.TEMPORARY, 0, (fs) => kvm.controller.files.onFileSystemReady(fs, fileName, content), function (err) {
            console.log(err);
        });
    },
    copyFile2: function (fs, srcFilePath, dstFile) {
        ...
        */
        ...
        window.resolveLocalFileSystemURL(srcFilePath, function (file) {
            console.log('copy file: %o', file);
            var documentsPath = fs.root;
            console.log(documentsPath);
            file.copyTo(srcFilePath, dstFile, function (res) {
    ...
    copyFile: function (srcDir, srcFile, dstDir, dstFile) {
        ...
        window.resolveLocalFileSystemURL(dstDir, function success(dirEntry) {
            dstEntry = dirEntry;
            window.resolveLocalFileSystemURL(srcDir + srcFile, function onSuccess(fileEntry) {
                fileEntry.copyTo(dstEntry, dstFile, function () {
