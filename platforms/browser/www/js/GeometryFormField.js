...
function GeometrieFormField(formId, settings) {
    ...
    this.bindEvents = function () {
        ...
        $('#goToGpsPositionButton').on('click', function () {
            ...
            var featureId = $('#featureFormular input[name=' + kvm.activeLayer.get('id_attribute') + ']').val(), feature = kvm.activeLayer.features[featureId], marker = kvm.map._layers[feature.layerId];
        ...
        $('#saveGpsPositionButton').on('click', function () {
            ...
            navigator.geolocation.getCurrentPosition(function (geoLocation) {
                navigator.notification.confirm('Neue Position:\n' + geoLocation.coords.longitude + ' ' + geoLocation.coords.latitude + '\nübernehmen?', function (buttonIndex) {
                    if (buttonIndex == 1) {
                        kvm.log('set new Position ' + geoLocation.coords.latitude + ' ' + geoLocation.coords.longitude, 4);
                        var feature = kvm.activeLayer.activeFeature, newGeom = feature.aLatLngsToWkx([{ lat: geoLocation.coords.latitude, lng: geoLocation.coords.longitude }]);
                ...
                }, 'GPS-Position', ['ja', 'nein']);
            }, function (error) {
                navigator.notification.confirm('Es kann keine GPS-Position bestimmt werden. Schalten Sie die GPS Funktion auf Ihrem Gerät ein, suchen Sie einen Ort unter freiem Himmel auf und versuchen Sie es dann noch einmal.', function (buttonIndex) {
                    if (buttonIndex == 1) {
        ...
        */
        $(document).on('geomChanged', function (event, options) {
            var feature = kvm.activeLayer.activeFeature, geom = options.geom, exclude = options.exclude;
            console.log('Trigger Funktion geomChanged: geom: %o und exclude: %s', geom, exclude);
            if (exclude != 'wkx') {
                var oldGeom = feature.newGeom, newGeom = geom;
                if (newGeom != oldGeom) {
            ...
            if (exclude != 'wkb') {
                var oldGeom = $('#featureFormular input[name=' + kvm.activeLayer.get('geometry_attribute') + ']').val(), newGeom = geom.toEwkb().toString('hex');
            ...
            if (exclude != 'wkt') {
                var oldGeom = $('#geom_wkt').val(), newGeom = geom.toWkt();
...
//# sourceMappingURL=GeometryFormField.js.map